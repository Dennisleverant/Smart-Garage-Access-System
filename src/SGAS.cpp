// Arduino core version: This project is built with the Arduino core version 2.0.6.
#include "common.h"
#include "SGAS.h"

void setup(){
	pinMode(INPUT_GD, INPUT_PULLUP);
	pinMode(OUTPUT_GD, OUTPUT);
	swSerial.begin(9600, SWSERIAL_8N1, INPUT_GD, OUTPUT_GD, true);
...
void transmit(byte* payload, unsigned int length){
	digitalWrite(OUTPUT_GD, HIGH); // pull the line high for 1305 micros so the door opener responds to the message
	delayMicroseconds(1305);
	digitalWrite(OUTPUT_GD, LOW); // bring the line low
